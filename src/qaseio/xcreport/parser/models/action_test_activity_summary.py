...
class ActivityType(Enum):
    ...
    ATTACHMENT_CONTAINER = "com.apple.dt.xctest.activity-type.attachmentContainer"
    NONE = "none"
...
class ActionTestActivitySummary:
    ...
    def from_report(cls, report: dict):
        ...
        return cls(
            ...
            helpers.enum_from_report(ActivityType, report.get("activityType"), dict(default=ActivityType.NONE)),
            helpers.list_from_report(ActionTestAttachment, report.get("attachments"), dict(default=[])),
            helpers.list_from_report(ActionTestActivitySummary, report.get("subactivities"), dict(default=[]))
        )
