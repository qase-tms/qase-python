...
class ActionTestSummary(ActionTestSummaryIdentifiableObject):
    ...
    def from_report(cls, report: dict):
        ...
        return cls(
            ...
            helpers.list_from_report(ActionTestActivitySummary, report.get("activitySummaries"), dict(default=[])),
            helpers.float_from_report(report.get("duration")),
            helpers.enum_from_report(TestStatus, report.get("testStatus"), dict(default=TestStatus.UNDEFINE))
        )


helpers.registry_subtype(ActionTestSummary, ActionTestSummaryIdentifiableObject)
